...
def getFinanceData(cmd=None):
    ...
    urlInfo = {
        ...
        303: 'http://compglobal.wisereport.co.kr/company/getFinStatement?cmp_cd={}-US&term=A&typ=BS&curr=LOC&en={}',
    }
        ...
        for key in reportType.keys():
            ...
            if cmd == 200:
                ...
                if len(stockInfo) != len(result):
                    ...
                    # exit(0)
            elif cmd == 301 or cmd == 302 or cmd == 303:
                agents = 2
                j = jobs()
                j.filetype = fileInfo[key]
                j.workDir = workDir
    ...
    try:
        for key in reportType.keys():
            ...
            if cmd and key != cmd:
                continue
            if cmd >= 300:
                stockInfo = detective_db.USNasdaqStocks.objects.filter(ticker='AAPL', listing='Y', fnguide_exist='Y')
            else:
